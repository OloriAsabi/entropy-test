...
const Home = () => {
  ...
  return (
    <>
      ...
      <Box minH={"100vh"} className='bg-home' w="full">
        {/*  hero section  */}
        <Flex h={["50%", "550px"]} w="90%" mx={"auto"} px={["1rem", "2rem"]} flexWrap="wrap" my={4}>
          <Box w={["100%", '50%']} display="flex" justifyContent={"center"} order={[2, 1]} flexDir="column">
            <Text as="h1" fontSize={[28, 40, 50]} color={'white'} fontWeight="bold" py={4}>
              Welcome 
            </Text>
            {/* <Text>The World is waiting for your to create your ideas, come share and create your world</Text> */}
            <Button fontWeight={"light"} bg="white" color={"black"} py="2" px="4" maxW={"max-content"} mt={4} onClick={() => setWriteModal(!writeModal)}>Create your post</Button>
          </Box>
          {/* <Image src='/logo.jpg' w={["full", '50%']} h="full"  objectFit={"contain"} order={[1, 2]} /> */}
        </Flex>
        <Text textAlign={"center"} color={'white'} fontSize={60} fontWeight={700}>Categories</Text>
        <Flex w={["90%", "max-content"]} mx="auto" px="4" p={5} alignItems="center" experimental_spaceX={[2]} flexWrap="wrap">
        ...
        <Flex w={"90%"} py={4} mx="auto" justifyContent="center">

          {
            blogsData?.filter(obj => category === "All" ? true : obj.category === category)?.map(obj =>
              <Link to={`/blog/${obj.id}`}>
                <Box w={["350px", "350px"]} rounded={"lg"} overflow="hidden" m={[2, 4]} h={["350px", "380px"]} color={dark ? "white" : "black"} bg={dark ? "black" : "white"} shadow="lg">
